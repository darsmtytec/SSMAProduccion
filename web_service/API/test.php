...
$bol = true;
$showSentiment = 'false';
$notweets = 10; //cantidad de tweets a mostrar
//</editor-fold>
...
if ($topics[0] != '') {
    ...
    for ($b = 0; $b < count($topics); $b++) {
        //var_dump($connection->get("https://api.twitter.com/1.1/search/tweets.json?q=" . $topics[$b] . "&count=" . $notweets));
        ...
        for ($a = 0; $a < count($phpArraySearch['statuses']); $a++) {
            if ($phpArraySearch['statuses'] != null && $phpArraySearch['statuses'][$a] != null && $phpArraySearch['statuses'][$a]['id_str'] != '') {
                ...
                $arraySearch[$c]["id_tweet"] = $phpArraySearch['statuses'][$a]['id_str'];
                ...
                //$array = array_merge($post,$arraySearch[$c]);
    ...
    for ($c = 0; $c < count($accounts); $c++) {
        ...
        for ($a = 0; $a < count($phpArrayAccount); $a++) {
            ...
            if ($phpArrayAccount[$a] != null && $phpArrayAccount[$a]['id_str'] != null) {
                ...
                $arraySearch[$c]["sentimiento"] = $sentiment;
                //</editor-fold>
                //$collection->insert($arraySearch);
                $collection ->update($arraySearch,array("upsert" => true));
...
//$array = array_merge($post,$arraySearch);
