...
function loadTweets (){
    ...
    $.post('web_service/twitter_APIConnect.php', dataItems, function(data, textStatus) {
...
function loadInstagram(){
    ...
    $.post('web_service/instagram_API.php', dataItems, function(data, textStatus) {
        ...
        for(i = 0; i < data.length; i++){
            cloneTweet = $('#todo-tasklist-instagram').clone().removeAttr('id').show();
            cloneTweet.find('[data-content="foto_perfil"]').attr('src',data[i].foto_perfil);
            cloneTweet.find('[data-content="nombre_usuario"]').text(data[i].nombre_usuario);
            cloneTweet.find('[data-content="text_clean"]').text(data[i].text_clean);
            var date = new Date(parseInt(data[i].created_time)*1000).toDateString();
            cloneTweet.find('[data-content="created_at"]').html('<i class="fa fa-instagram"></i>'+date);

            cloneTweet.data('id_post',data[i].id_post);
            cloneTweet.data('url',data[i].url);
            cloneTweet.data('location',data[i].location);

            if(parseInt(data[i].likes) != 0){
                cloneTweet.find('[data-content="likes"]').html('<i class="font-red-haze fa fa-heart"></i>' + data[i].likes);
            }else{
                cloneTweet.find('[data-content="likes"]').html('<i class="fa fa-heart"></i>'+ data[i].likes);
            }
            if(parseInt(data[i].comments['count']) != 0){
                cloneTweet.find('[data-content="count"]').html('<i class="font-blue fa fa-commenting"></i>'+ data[i].comments.count);
            }else{
                cloneTweet.find('[data-content="count"]').html('<i class="fa fa-commenting"></i>'+ data[i].comments.count);
            }
...
function loadTumblr () {
    ...
    $.post('web_service/tumblr_API.php', dataItems, function (data, textStatus) {
        ...
        $('[data-content="social-tumblr-count"]').text(data.length);
